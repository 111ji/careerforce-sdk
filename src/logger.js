...
module.exports = {
    ...
    log: function(type, data, objectId) {
        var content = {};
        content.type = type;
        content.product = "111ji-mp";

        if (typeof(data) == "string") {
            if (this.pages[data] != null && this.pages[data] != undefined)
                content.data = this.pages[data];
            else
                content.data = data;
        } else
            content.data = this.jsonToString(data);
    ...
    upload: function(content) {
        this._httpClient.fetch("https://api.careerforce.cn/111ji/u/api/logger", content).then(function() {
            if (console)
